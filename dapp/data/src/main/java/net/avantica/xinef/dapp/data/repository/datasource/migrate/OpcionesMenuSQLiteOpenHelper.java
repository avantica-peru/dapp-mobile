package net.avantica.xinef.dapp.data.repository.datasource.migrate;
...
public class OpcionesMenuSQLiteOpenHelper extends SQLiteOpenHelper {

    public OpcionesMenuSQLiteOpenHelper(Context context) {
        super(context, UbigeoDataBase.NAME + ".db", null, UbigeoDataBase.VERSION);
    }

    @Override
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        sqLiteDatabase.execSQL("DROP TABLE IF EXISTS UBIGEO;");


        String createTable = "CREATE TABLE IF NOT EXISTS `ubigeo` (\n" +
                "  `codDpto` TEXT ,\n" +
                "  `codProv` TEXT,\n" +
                "  `codDist` TEXT,\n" +
                "  `descripcion` TEXT,\n" +
                "  PRIMARY KEY (`codDpto`,`codProv`,`codDist`)\n" +
                ")";

//            db.execSQL("CREATE TABLE OPCIONESMENU " +
...
//                    "Tipo TEXT)");
    ...
    private void populateUbigeo(SQLiteDatabase db) {
        String table = "INSERT INTO 'ubigeo' ('codDpto', 'codProv', 'codDist', 'descripcion') VALUES";

        String amazonas = table +
                "('01', '00', '00', 'AMAZONAS'),"+
